...
containerID=$(docker ps -q --filter ancestor=kubetoolsca/krs-docker-extension:latest | head -n 1)
if [ -n "$containerID" ]; then
  ...
  echo "No running container found for kubetoolsca/krs-docker-extension:latest"
fi