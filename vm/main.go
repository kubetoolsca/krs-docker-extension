...
func main() {
	...
	flag.StringVar(&socketPath, "socket", "/run/guest-services/krs-extension.sock", "Unix domain socket to listen on")
	flag.Parse()
...
// ready checks whether krs-extension is ready or not by querying localhost:9080.
...
func (t *TTYD) Start(theme Theme) error {
	...
	if !t.IsServed() {
		...
		t.serve = cmd.Process
		log.Println("started krs serve...")
	}
	if !t.IsStarted() {
		args := []string{"-W", "-u", "1000", "-g", "1000", "-t", "titleFixed='krs'"}
...
func (t *TTYD) IsRunning() bool {
	...
	url := "http://localhost:3986/health" // "krs" is the name of the service defined in docker-compose.yml
